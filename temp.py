...
def oled_display():
	...
	while loop:
		if alert == 1:
			...
			draw.text((0,30), 'Mixer:   ', font=font, fill=255)
			draw.text((70,20), '%.2f'%(1.0/delay+random.uniform(-5,5)), font=font, fill=255)
			draw.text((70,30), '%.2f'%(1.0/delay+random.uniform(-5,5)), font=font, fill=255)
			draw.text((110,20), 'm^3', font=font, fill=255)
			draw.text((110,30), 'm^3', font=font, fill=255)
			#get temperaute and pressusre	
			temp = bmp.readTemperature()
			pressure = bmp.readPressure()
			draw.text((0,40), 'Temperature:', font=font, fill=255)
			draw.text((0,50), 'Pressure:   ', font=font, fill=255)
			draw.text((70,40), '%.2f'% temp , font=font, fill=255)
			draw.text((70,50), '%.2f'%(pressure / 100.0), font=font, fill=255)
			draw.text((110,40), 'C', font=font, fill=255)
			draw.text((110,50), 'hPa', font=font, fill=255)
